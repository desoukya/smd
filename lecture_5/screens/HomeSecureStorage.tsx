...
export default function Home() {
  ...
  const getTodosFromSecureStore = async () => {
    const todos = await SecureStore.getItemAsync('todoList');
  ...
  const saveTodosToSecureStore = async (todos: Todo[]) => {
    await SecureStore.setItemAsync('todoList', JSON.stringify(todos));
    setTodoList(todos);
  };  

  useEffect(() => {
    getTodosFromSecureStore();
  }, []);
  ...
  const handleSubmit = async (): Promise<void> => {
    if (!todo) {
      ...
      await saveTodosToSecureStore(todos);
      setTodo('');
    }
  };
  ...
  const removeItem = async (idx: number): Promise<void> => {
    ...
    await saveTodosToSecureStore(newToDoList);
  };
